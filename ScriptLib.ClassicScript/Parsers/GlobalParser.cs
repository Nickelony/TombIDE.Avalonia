...
namespace ScriptLib.ClassicScript.Parsers
{
	public static class GlobalParser
	{
		...
		{
			string commandKey = CommandParser.GetCommandKeyFromOffset(document, offset);
		...
		public static int GetNextFreeIndex(TextDocument document, int offset, string commandKey)
		{
			...
			if (StringExtensions.BulkStringComparision(commandKey, StringComparison.OrdinalIgnoreCase,
				...
				"Organizer", "Parameters", "TestPosition", "TriggerGroup", "Plugin"))
			{
				...
				{
					DocumentLine? sectionStartLine = DocumentParser.GetSectionStartLineFromOffset(document, offset);
		...
		private static IEnumerable<int> GetTakenIndicesList(TextDocument document, string commandKey, int loopStartLine)
		{
			for (int i = loopStartLine; i <= document.LineCount; i++)
			{
				...
				string? command = CommandParser.GetCommandKeyFromOffset(document, processedLine.Offset);
